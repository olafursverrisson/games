...
function update(dt) {
  ...
  for (const p of players) {
    ...
    if (p.jumpQueued && onGround && p.jumpCooldown <= 0 && p.tumble <= 0) {
      p.vy = -760;
      p.jumpCooldown = 0.28;
...
window.addEventListener("keydown", (event) => {
  ...
  const p = players[keyMap[key]];

  // Press key in air while holding -> throw/lob toward hoop.
  if (p.holding && p.y < FLOOR_Y - 2) {
    releaseBallFromPlayer(p);
  }

  // Hold still raises hands/moves, but jumping is queued only on press.
  p.wantHold = true;
  if (p.y >= FLOOR_Y - 1) {
    p.jumpQueued = true;
  }
